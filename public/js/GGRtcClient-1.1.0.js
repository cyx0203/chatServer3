...
class GGRtcClient {
	...
	// 停止录制 - 本地
	_stopLocalRecord () {
		if(!this._mediaLocalRecorder) return;
	...
	_playLocalRecord (videoDomId)
	{
		...
		if(!this._recLocalBuffer) return;
	...
	// 停止录制 - 远端
	_stopRemoteRecord () {
		if(!this._mediaRemoteRecorder) return;
	...
	{
		if(!videoDomId) return;
		if(!this._recRemoteBuffer) return;
